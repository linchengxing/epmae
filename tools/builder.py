...
def data_augmentation_builder(config):
    if config.get('train_transforms') is not None:
        train_transforms = []
        if config.train_transforms.Rotate:
            train_transforms.append(data_transforms.PointcloudRotate())
        if config.train_transforms.Jitter:
            train_transforms.append(data_transforms.PointcloudJitter())
        if config.train_transforms.ScaleAndTranslate:
            train_transforms.append(data_transforms.PointcloudScaleAndTranslate())
        train_transforms = transforms.Compose(train_transforms)
    else:
        # identity mapping
        train_transforms = None
    
    if config.get('test_transforms') is not None:
        test_transforms = []
        if config.test_transforms.Rotate:
            test_transforms.append(data_transforms.PointcloudRotate())
        if config.test_transforms.Jitter:
            test_transforms.append(data_transforms.PointcloudJitter())
        if config.test_transforms.ScaleAndTranslate:
            test_transforms.append(data_transforms.PointcloudScaleAndTranslate())
