...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if args.resume:
        ...
        if args.ckpts is not None:
            base_model.load_model_from_ckpt(args.ckpts, logger)
        else:
    ...
    print_log('Number of model parameters: {}M'.format(sum([p.data.nelement() for p in base_model.parameters()]) / 10 ** 6), logger = logger)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        if train_writer is not None:
            ...
            train_writer.add_scalar('Metric/ACC', losses.avg(1), epoch)
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            if args.vote:
                if metrics.acc > 92.7 or (better and metrics.acc > 91):
